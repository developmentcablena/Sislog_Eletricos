...
Partial Public Class Autorizar
    ...
    Protected Sub gvCadastros_RowCommand(sender As Object, e As GridViewCommandEventArgs)
        ...
        Select Case commandName
            ...
            Case "abrirModal"
                ...
                'ScriptManager.RegisterStartupScript(Me, Me.GetType(), "SucessoEmail2", "abrirModalAutorizacao(); abrirModalEmbarque();", True)
    ...
    Private Sub RejeitarCadastro(ByVal cadastroID As Integer)
        Using conn As New SqlConnection(connectionString)
            Dim sql As String = "UPDATE tb_Cadastro SET Status= 3 WHERE CadastroID= @id"
            Using cmd As New SqlCommand(sql, conn)
                cmd.Parameters.AddWithValue("@id", cadastroID)
                Try
                    conn.Open()
                    cmd.ExecuteNonQuery()
                    conn.Close()
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "sucesso", $"alert('Cadastro Nº {cadastroID} rejeitado!'), abrirModalAutorizacao();", True)
                    CarregarDados()
                Catch ex As Exception
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Alerta", "alert('Erro ao cadastrar: " & ex.Message.Replace("'", "\'") & "')", True)
                Finally
    ...
    Private Sub AbrirModal(ByVal cadastroID As Integer, ByVal tipoCadastro As String)
        ...
        ElseIf tipoCadastro = "Recebimento" Then
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "SucessoEmail2", "abrirModalAutorizacao(); abrirModal2();", True)
        End If
    End Sub
